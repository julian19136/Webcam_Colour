...
int julianIterations(vec2 z, vec2 c)
{
...
void main()
{
    fragColor = vec4((moveMent.x) / windowSize.x, moveMent.y / windowSize.y, 5, 0.2);
    
    vec2 mousePosFragCoord = vec2(moveMent.x, windowSize.y - (moveMent.y));
    vec2 c = fragCoordToComplex(mousePosFragCoord);
    vec2 z = fragCoordToComplex(gl_FragCoord.xy);
    int numIterations = julianIterations(z, c);
